...
if (strlen($_SESSION['empid'] == 0)) {
  ...
  if (isset($_POST['submit'])) {
    ...
    if (($_POST['aptserv']) == 0) {
      ...
      $ret = mysqli_query($con, "DELETE FROM tblinvoice WHERE soID='$cid';");
      $aptservID = $_POST['aptserv'];

      for ($i = 0; $i < count($aptservID); $i++) {
        $svid = $aptservID[$i];
        $ret = mysqli_query($con, "INSERT into tblinvoice(InvoiceID,soID,ServiceID) values('$invoice','$cid','$svid');");
      }
    }

    echo '<script>alert("Service order has been update!")</script>';
    echo "<script>window.location.href='transaction-view.php?viewid=$cid'</script>";
  }
}
?>
...
<body>
  ...
  <?php
    $cid = $_GET['editid'];
    $ret = mysqli_query($con, "SELECT tblcustomers.CustID, tblcustomers.FirstName, tblcustomers.LastName, tblserviceorder.CustomerID, tblserviceorder.soID, tblserviceorder.EmployeeID, tblserviceorder.custType, tblserviceorder.PaymentStatus, tblserviceorder.Status, tblserviceorder.InvoiceID, tblserviceorder.CreationDate,tblemployee.EmployeeID, tblemployee.FirstName as empFname, tblemployee.LastName as empLname
    ...
    <div class="pagetitle">
      ...
      <nav>
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item"><a href="transaction-view.php?viewid=<?php echo htmlentities($cid); ?>">Service Order No. <?php echo $cid ?></a></li>
          <li class="breadcrumb-item active">Update Service Order</a></li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
              ...
              <div class="card-body">
                ...
                <form method="POST">
                  ...
                  <div class="row g-3">
                    ...
                    <input type="text" class="form-control" name="invoice" id='invoice' value="<?php echo $row['InvoiceID']; ?>" hidden>
                    ...
                    <div class="col-md-6">
                      <label class="form-label">Service Order Status<i>*</i></label>
                      <input type="text" class="form-control" value="<?php echo $row['Status']; ?>" disabled>
                      
                    </div>

                    <div class="col-md-6">
                      <label class="form-label">Payment Status<i>*</i></label>
                  ...
                  <div align="center">
                    ...
                    <a href="transaction-view.php?viewid=<?php echo htmlentities($cid); ?>" class="add-btn">CANCEL</a>
                  </div>
                </form>
              </div>
            </div>
